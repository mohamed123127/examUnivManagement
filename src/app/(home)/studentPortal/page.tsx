...
const Page = () => {
  ...
  useEffect(()=>{
    const isAuthorized = user?.role == 'Étudiant';
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/students/"+user?.groupId+"/ExamsSchedule")
      .then((res) => res.json())
                    ...
                    <td key={date + key} className="border border-gray-300 px-2 py-2">
                      {exam ? (
                        <div className="bg-blue-100 p-2 rounded shadow text-left">
                          ...
                          <p className="text-sm">
                            <span className="font-semibold">Supervisors:</span> {exam.supervisions.join(", ")}
                          </p>
                        </div>
                      ) : null}
                    </td>
